    ...
    func getProfileAndBucket(handler: @escaping (_ error:String) -> Void) {
        PPManager.sharedInstance.PPusersvc.getProfile {  succeeded, response, user in
            if(!succeeded) {
                ...
                /*
                 PPManager.sharedInstance.PPdatasvc.openBucket(PPManager.sharedInstance.PPusersvc.myDataStorage, andUsers: [PPManager.sharedInstance.PPusersvc.userId, nil], public:false, handler:^(NSError* error) {
